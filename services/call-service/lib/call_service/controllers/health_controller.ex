defmodule CallService.HealthController do
  use Phoenix.Controller, formats: [:json]

  def index(conn, _params) do
    json(conn, %{status: "healthy", service: "call-service", version: "1.0.0"})
  end
  ...
  defp check_mongo do
    case Mongo.command(:call_mongo, %{ping: 1}) do
      {:ok, _} -> :ok
      _ -> :error
    end
  rescue
  ...
  defp check_redis do
    case Redix.command(:call_redis, ["PING"]) do
      {:ok, "PONG"} -> :ok
      _ -> :error
    end
  rescue
